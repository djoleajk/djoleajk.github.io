...
PeriodTracker.prototype.setupEventListeners = function() {
    ...
    if (startSurveyBtn) {
        startSurveyBtn.addEventListener('click', () => this.surveyModule.startDailySurvey(true));
    }
    ...
    // Groq anketa
    document.getElementById('survey-next-btn').addEventListener('click', () => this.surveyModule.nextSurveyQuestion());
    document.getElementById('survey-submit-btn').addEventListener('click', () => this.surveyModule.submitSurvey());
    document.getElementById('survey-skip-btn').addEventListener('click', () => this.surveyModule.skipSurvey());
    document.getElementById('survey-prev-btn').addEventListener('click', () => this.surveyModule.prevSurveyQuestion());
    document.getElementById('survey-text-input').addEventListener('input', () => {
        ...
        const isLastQuestion = this.surveyModule.currentQuestionIndex === this.surveyModule.surveyQuestions.length - 1;
