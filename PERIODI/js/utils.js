...
// Brze Akcije
PeriodTracker.prototype.showQuickActions = function() {
    const activeCycle = this.getActiveCycle();
    let actions = [];

    if (!activeCycle) {
        actions = [
            { icon: 'fa-play', text: 'Počni svoj ciklus', action: () => this.showCycleModal() },
            { icon: 'fa-notes-medical', text: 'Zabeleži svoje simptome', action: () => this.switchTab('symptoms') }
        ];
    } else {
        actions = [
            { icon: 'fa-tint', text: 'Zabeleži svoju menstruaciju', action: () => this.showCycleModal() },
            { icon: 'fa-notes-medical', text: 'Zabeleži svoje simptome', action: () => this.switchTab('symptoms') },
...
PeriodTracker.prototype.showQuickActionsModal = function(actions) {
    const modalHTML = `
