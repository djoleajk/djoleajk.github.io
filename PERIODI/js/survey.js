// Funkcije za Groq Anketu

PeriodTracker.prototype.startDailySurvey = async function(force = false) {
...
PeriodTracker.prototype.generateSurveyQuestions = async function() {
    ...
    try {
        ...
        const cycleDay = activeCycle ? this.getCurrentCycleDay() : null;
        const cyclePhase = activeCycle ? this.getCyclePhase(cycleDay) : 'Nema aktivnog ciklusa';
...
4. Emocionalno stanje - OVO MORA BITI TYPE "checkbox" sa opcijama kao što su: ["Srećna", "Umorna", "Pod stresom", "Anksiozna", "Raspoložena", "Tužna", "Iritirana", "Mirna", "Nervozna", "Opuštena"] - POSTAVLJENO DIREKTNO U DRUGOM LICU JEDNINE ŽENSKOJ OSOBI
5. Dodatni komentari ili napomene - OVO MORA BITI TYPE "checkbox" sa opcijama kao što su: ["Nema napomena", "Osećam se bolje nego juče", "Potrebno je više odmora", "Osećam se loše", "Sve je u redu", "Imam potrebu za podrškom"] - POSTAVLJENO DIREKTNO U DRUGOM LICU JEDNINE ŽENSKOJ OSOBI
...
[
  {"question": "Tekst pitanja NA SRPSKOM POSTAVLJEN DIREKTNO U DRUGOM LICU JEDNINE ŽENSKOJ OSOBI", "type": "scale" ili "text" ili "checkbox", "options": ["opcija1", "opcija2", ...]},
  ...
]
...
Pitanja 4 i 5 MORAJU biti type "checkbox" sa ponuđenim opcijama, NE type "text".
...
- Za pitanje o emocionalnom stanju (pitanje 4) MORA biti type "checkbox" sa opcijama kao što su: ["Srećna", "Umorna", "Pod stresom", "Anksiozna", "Raspoložena", "Tužna", "Iritirana", "Mirna", "Nervozna", "Opuštena"]
- Za pitanje o dodatnim komentarima (pitanje 5) MORA biti type "checkbox" sa opcijama kao što su: ["Nema napomena", "Osećam se bolje nego juče", "Potrebno je više odmora", "Osećam se loše", "Sve je u redu", "Imam potrebu za podrškom"]
...
PeriodTracker.prototype.getDefaultSerbianQuestions = function(cyclePhase, cycleDay) {
    return [
        {
...
PeriodTracker.prototype.displayCurrentQuestion = function() {
    ...
    // Show/hide buttons for scale questions
    this.updateSurveyButtons();
};

PeriodTracker.prototype.initProgressSteps = function() {
...
PeriodTracker.prototype.updateProgressSteps = function() {
    for (let i = 0; i < this.surveyQuestions.length; i++) {
...
PeriodTracker.prototype.updateSurveyButtons = function() {
    const nextBtn = document.getElementById('survey-next-btn');
    const submitBtn = document.getElementById('survey-submit-btn');
    const skipBtn = document.getElementById('survey-skip-btn');
    const isLastQuestion = this.currentQuestionIndex === this.surveyQuestions.length - 1;
...
PeriodTracker.prototype.prevSurveyQuestion = function() {
...
PeriodTracker.prototype.nextSurveyQuestion = function() {
...
PeriodTracker.prototype.submitSurvey = function() {
    // Get last answer if it's text input or checkbox
    const currentQuestion = this.surveyQuestions[this.currentQuestionIndex];
    const textInput = document.getElementById('survey-text-input');
...
PeriodTracker.prototype.processSurveyAnswers = function() {
    ...
    this.symptoms = this.symptoms.filter(s => s.date !== today);
    ...
    this.generateSurveySummary(symptomEntry).then(summary => {
...
PeriodTracker.prototype.showSurveySummary = function() {
    const contentEl = document.getElementById('survey-content');
...
PeriodTracker.prototype.skipSurvey = function() {
    this.closeModal();
    this.surveyActive = false;
};
